                    ...
                    <div class="card-body">
                        <form action="{{ route('jenis-produk.update', $jenisProduk->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $jenisProduk->name) }}" required autofocus>
                                ...
                                <div class="border rounded p-2" style="max-height: 150px; overflow-y: auto;">
                                    @foreach ($kategoris as $kategori)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="kategoris[]" value="{{ $kategori->id }}" id="kategori-edit-{{ $kategori->id }}"
                                                {{-- Logika untuk mencentang kategori yang sudah terhubung dengan jenis produk ini --}}
                                                {{ in_array($kategori->id, old('kategoris', $jenisProduk->kategoris->pluck('id')->toArray())) ? 'checked' : '' }}>
